...
import { HomeIcon, BookOpenIcon, PencilSquareIcon, UserCircleIcon, Squares2X2Icon, MoonIcon, SunIcon } from './icons/Icons';
...
export const Navbar: React.FC<NavbarProps> = ({ navigateTo }) => {
  ...
  const mobileNavLinks = [
      ...
      { label: 'Profile', action: () => { window.location.hash = '/profile'; navigateTo({ name: 'profile', user: sampleUser }); }, icon: UserCircleIcon },
  ];
    ...
    <>
      ...
      <header className={`fixed top-0 left-0 right-0 z-50 hidden md:block transition-all duration-300 ${isScrolled ? 'bg-surface/80 dark:bg-dark-surface/80 backdrop-blur-lg shadow-soft dark:border-b dark:border-dark-border' : 'bg-transparent'}`}>
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <a href="#/" onClick={() => navigateTo({ name: 'home' })} className="font-sans font-bold text-2xl text-primary dark:text-gray-100 tracking-tighter">
            Aetherium
          </a>
