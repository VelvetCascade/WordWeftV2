...
import { ArrowLeftIcon, BookmarkIcon, CheckCircleIcon, LockClosedIcon, StarIcon, PlusIcon, PencilIcon, TrashIcon, ShareIcon, EyeIcon, HeartIcon, ChatBubbleIcon } from '../components/icons/Icons';
      ...
      <div className="flex items-center gap-4 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-4 mt-2 text-xs text-gray-400 dark:text-gray-500">
            <span className="flex items-center gap-1"><EyeIcon className="w-3 h-3" /> {chapter.viewCount || 0}</span>
            <span className="flex items-center gap-1"><HeartIcon className="w-3 h-3" /> {chapter.likeCount || 0}</span>
            <span className="flex items-center gap-1"><ChatBubbleIcon className="w-3 h-3" /> {chapter.commentCount || 0}</span>
          </div>
        </div>
      </div>
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: book.title,
          text: `Check out "${book.title}" on WordWeft!`,
          url: url,
        });
      } catch (err) {
      ...
      // Ideally use a toast here, but alert is acceptable for now per plan
    ...
    <div className="bg-white dark:bg-dark-surface">
      ...
      <div className="sticky top-0 z-30 bg-white/80 dark:bg-dark-surface/80 backdrop-blur-md border-b border-gray-200 dark:border-dark-border">
        <div className="container mx-auto px-4 sm:px-6 h-20 flex items-center justify-between">
          ...
          <button onClick={handleShare} className="ml-4 text-gray-400 dark:text-gray-500 hover:text-accent transition-colors">
          ...
          <div className="md:col-span-2 lg:col-span-3">
            ...
            <div className="flex items-center gap-4 mb-4">
              ...
              <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400">
                ...
                <span className="text-sm font-medium">{book.viewCount || 0}</span>
              </div>
              <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400">
                <HeartIcon className="w-4 h-4" />
                <span className="text-sm font-medium">{book.likeCount || 0}</span>
              </div>
              <div className="flex items-center gap-1 text-gray-500 dark:text-gray-400">
                <ChatBubbleIcon className="w-4 h-4" />
                <span className="text-sm font-medium">{book.commentCount || 0}</span>
              </div>
            </div>
