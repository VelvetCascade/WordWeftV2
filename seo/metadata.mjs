...
export function parseRoute(input) {
  try {
    ...
    if (/^\/(auth|reset-password|profile|edit-profile|notifications|search|feedback|hooks|events|challenges|community)(\/|$)/.test(path) || /^\/write(?:\/|$)/.test(path)) return { kind: 'private', path };
